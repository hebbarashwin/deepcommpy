...
def crisp_rnn_test(polar, device, net=None, config=None):
    ...
    with torch.no_grad():
        for k in range(num_test_batches):
            msg_bits = 2*torch.randint(0, 2, (config['test_batch_size'], config['K']), dtype=torch.float) - 1
    ...
    with torch.no_grad():
        for k in range(num_test_batches):
            ...
            for snr_ind, snr in enumerate(snr_range):
                ...
                ber_CNN = errors_ber(msg_bits, decoded_CNN_msg_bits.sign()).item()
                bler_CNN = errors_bler(msg_bits, decoded_CNN_msg_bits.sign()).item()
